...
<div class="bg-surface-container-lowest rounded-2xl shadow-xl p-8 border border-outline-variant/30">
    <h2 class="text-xl font-bold text-on-background mb-1">Create account</h2>
    <p class="text-on-surface-variant text-sm mb-6">Start organizing your work today</p>

    <form wire:submit="register" class="space-y-4">
        <div>
            <label class="block text-sm font-medium text-on-surface mb-1">Name</label>
            <input wire:model="name" type="text" autocomplete="name" placeholder="Your name"
        ...
        <div>
            ...
            <input wire:model="password_confirmation" type="password" autocomplete="new-password" placeholder="Repeat password"
                class="w-full rounded-xl border border-outline-variant bg-surface-container px-4 py-2.5 text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary" />
    ...
    <p class="text-center text-sm text-on-surface-variant mt-6">
        Already have an account? <a href="{{ route('login') }}" wire:navigate class="text-primary font-medium hover:underline">Sign in</a>
    </p>
</div>