...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function invite(): void {
        ...
        if (!$user) { $this->error = 'No user found with that email.'; return; }
        if ($user->id === $this->project->owner_id) { $this->error = 'Owner cannot be added as a member.'; return; }
...
<div class="p-4 md:p-6 space-y-6 max-w-2xl">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl font-bold text-on-background">Members: {{ $project->name }}</h1>
    </div>

    <div class="bg-surface-container rounded-2xl p-5 space-y-4">
        <h2 class="font-semibold text-on-background">Invite Member</h2>
        ...
        <div class="flex gap-3 flex-wrap">
            ...
            <button wire:click="invite"
                class="px-4 py-2 bg-primary text-on-primary rounded-xl text-sm font-medium hover:bg-primary/90">Invite</button>
        </div>
    </div>

    <div class="bg-surface-container rounded-2xl p-5 space-y-3">
        <h2 class="font-semibold text-on-background">Current Members</h2>
        ...
        @empty
        <p class="text-sm text-on-surface-variant text-center py-4">No members yet.</p>
        @endforelse
    </div>
</div>
</div>
