...
<body class="bg-background text-on-background overflow-hidden">
    ...
    <div class="h-screen flex overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        <div x-show="sidebarOpen" x-cloak x-transition:enter="transition-opacity ease-linear duration-200"
            ...
            class="fixed inset-0 bg-slate-900/60 z-40 lg:hidden"></div>
        ...
        <main class="flex-1 flex flex-col min-w-0 lg:ml-64">
            ...
            {{-- ---- TOP HEADER ---- --}}
            <header
                class="bg-white/90 backdrop-blur-xl h-16 sticky top-0 z-40 flex items-center justify-between px-4 sm:px-6 lg:px-8 border-b border-slate-100/80 shrink-0">
                <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                    ...
                    <div class="relative hidden sm:block w-full max-w-xs">
                        ...
                        <input
                            ...
                            placeholder="Búsqueda global..." type="text" />
                    </div>
                ...
                <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                    <div class="hidden sm:flex items-center gap-1 pr-3 border-r border-slate-100">
                        ...
                        <button
                            class="p-2 text-slate-400 hover:bg-surface-container-low rounded-lg transition-colors relative">
                            <span class="material-symbols-outlined text-xl">notifications</span>
                            <span
                                class="absolute top-2 right-2 w-2 h-2 bg-indigo-500 rounded-full border-2 border-white"></span>
                        </button>
                    </div>
                    <button
                        class="hidden sm:block bg-primary text-on-primary px-4 lg:px-5 py-1.5 rounded-full text-sm font-semibold shadow-md shadow-indigo-600/20 hover:scale-105 transition-transform active:scale-95">
                        Desplegar
                    </button>
                    <div
                        class="h-8 w-8 rounded-full overflow-hidden bg-indigo-500 flex items-center justify-center shrink-0">
                        @auth
            ...
            {{-- ---- MOBILE BOTTOM NAV ---- --}}
            <nav
                class="md:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur border-t border-slate-200 z-40 flex items-stretch">
                <a href="{{ route('kanban') }}"
